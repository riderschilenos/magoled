...
<div class="min-w-screen bg-gray-100 flex items-center justify-center px-5 py-4">
        ...
        <div class="md:flex w-full">
            ...
            <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                <div class="text-center mb-10">
                    <h1 class="font-bold text-3xl text-gray-900">REGÍSTRA Y ADMINISTRA TU TIENDA ONLINE</h1>
                   
                </div>
                <div>
                    {!! Form::open(['route'=>'tiendas.store', 'autocomplete'=>'off']) !!}
                    
                    {!! Form::hidden('user_id',auth()->user()->id) !!}
                    <div class="flex justify-between">
                        <div class="mb-2">
                            <label for="" class="text-xs font-semibold px-1">Nombre de la tienda</label>
                            <div class="flex">
                                <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                                {!! Form::label('nombre', 'Nombre',["class"=>'hidden']) !!}
                                {!! Form::text('nombre', null , ['class' => ' -ml-10 pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500'.($errors->has('nombre')?' border-red-600':'')]) !!}

                            </div>
                        </div>
                       
                        <div class="justify-start ">
                            <label for="" class="text-xs font-semibold">Disciplina</label>
                            <div class="">
                            
                                {!! Form::select('disciplina_id', $disciplinas, null , ['class'=>' -ml-10 pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500']) !!}
                            </div>
                        </div>
                    </div>
                    <div class="">
                        ...
                        {!! Form::label('slug2', 'URL:') !!}
                        {!! Form::text('slug2', null , ['readonly'=>'redonly','class' => 'form-input block  mt-1 w-full pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500','placeholder'=>'www.riderschilenos.cl/tiendas/']) !!}
                    
                    </div>
                    
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            <label for="" class="text-xs font-semibold px-1">Fono/Whatsapp</label>
                            <div class="flex">
                                <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-email-outline text-gray-400 text-lg"></i></div>
                                {!! Form::text('fono', null , ['class' => 'w-full -ml-10 pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500'.($errors->has('fono')?' border-red-600':'')]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            <label for="" class="text-xs font-semibold px-1">Ubicación</label>
                            <div class="flex">
                                <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-email-outline text-gray-400 text-lg"></i></div>
                                {!! Form::text('ubicacion', null , ['class' => 'w-full -ml-10 pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500'.($errors->has('ubicacion')?' border-red-600':'')]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            {!! Form::submit('Enviar Regístro', ['class'=>'block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold']) !!}
                        </div>
                    </div>
...
<x-slot name="js">
    <script src="{{asset('js/vendedor/tiendas/form.js')}}"></script>
