        ...
        <section class="bg-white md:py-4 mb-8 ">
            <div class=" max-w-7xl mx-auto sm:px-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="text-gray-100 items-center my-auto bg-gray-700 p-4 rounded-2xl mx-2">
                    ...
                    @if ($evento->type=='sorteo')
                        <div>
                            <div class="inline-block mb-2 ms-[calc({{$inscritos*100/$evento->limite}}%-1.25rem)] py-0.5 px-1.5 bg-gray-100 border border-blue-200 text-xs font-medium text-blue-600 rounded-lg ">{{number_format($inscritos*100/$evento->limite,1)}}%</div>
            ...
            @if($fechas->where('end_sell','!=',null)->count()>0)
                ...
                <script>
                    function updateCountdownClock2() {
                        var startSellTime2 = new Date( <?php echo json_encode($evento->fechas->where('end_sell', '!=', null)->first()->created_at->addHours(4073)) ?>);
                        var currentTime = new Date();
